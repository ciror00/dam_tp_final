...
routerRiego.get('/:idDispositivo', function(req, res) {
    pool.query('Select * from Log_Riegos where valvula_id=? order by fecha desc', [req.params.idDispositivo], function(err, result, fields) {
        if (err) {
...
routerRiego.post('/agregar', function(req, res) {
    pool.query('Insert into Log_Riegos (fecha,apertura,valvula_id) values (?,?,?)', [req.body.fecha, req.body.valor, req.body.electrovalvulaId], function(err, result, fields) {
        if (err) {
            res.send(err).status(400);
            return;
        }
